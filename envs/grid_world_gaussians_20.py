'''sensing range = 1, obstacles and discrete reward based on the difference between the sums of pdfs before and after the action'''
import numpy as np
import pygame
...
class GridWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        sensing_old = observation_old.get("sensing")
        ...
        if np.any(obst_obs):
            ...
            if np.sum(sensing_old) < np.sum(sensing):
                reward = 1
            elif np.sum(sensing_old) > np.sum(sensing):
                reward = -1
            else:
                reward = 0
