...
class TestEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        for idx in range(self.robots_num):
            ...
            mates = [r - self._robot_positions[idx, :] for r in np.delete(self._robot_positions, idx, 0)]

            dict_obs = {"grid": np.expand_dims(norm_obs, 0), "mates": mates}
            observations.append(dict_obs)
    ...
    def _get_info(self):
        return {
            "positions": [self._robot_positions[i] for i in range(self.robots_num)]
        }
    ...
    def reset(self, seed=None, options=None):
        ...
        self._robot_positions = np.expand_dims((self._robot_position), 0)
        ...
        for i in range(self.mates_num):
            ready = False
            while not ready:
                m = 1 + (self.width-2) * np.random.rand(1, 2)
                ready = np.linalg.norm(m - self._robot_position) > 1.0
            self._mates_positions.append(m)
            self._robot_positions = np.concatenate((self._robot_positions, m), 0)
    ...
    def step(self, action, idx):
        ...
        self._robot_positions[idx] = np.clip(self._robot_positions[idx] + action*self.dt, 0.5, self.width-0.5)
        ...
        reward = (100*self.grid[imin:imax, jmin:jmax]).sum()

        # reward -= self.t
        for x_j in np.delete(self._robot_positions, idx, 0):
            d = np.linalg.norm(self._robot_positions[idx] - x_j)
    ...
    def _render_frame(self):
        ...
        # Draw the robots + sensing region
        for i in range(self.robots_num):
            pygame.draw.circle(
                canvas,
                (0, 255, 0),
                self._robot_positions[i] * pix_square_size / self.discretize_precision,
                pix_square_size / 3 * 5,
            )
            ...
            pygame.draw.rect(
                ...
                (255, 0, 0),
                pygame.Rect(
                    pix_square_size * np.array([self._robot_positions[i, 0]/self.discretize_precision-self.obs_shape/2+self.i_start, self._robot_positions[i, 1]/self.discretize_precision-self.obs_shape/2+self.i_start]),
