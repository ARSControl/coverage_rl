'''sensing range = 1 and discrete reward based on the difference between the sum of pdfs in the observation space before and after the action '''
import numpy as np
import pygame
...
class GridWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.old_pdf_obs = self._get_obs()
        ...
        self.new_pdf_obs = self._get_obs()

        #reward depends on the difference
        if np.sum(self.old_pdf_obs) < np.sum(self.new_pdf_obs):
            reward = 1
        elif np.sum(self.old_pdf_obs) > np.sum(self.new_pdf_obs):
            reward = -1
        else:
            reward = 0
