...
class MultiObsEnv(gym.Env):
    ...
    def __init__(self, robot_range=3, robots_num=3, sigma=2, discr=0.2, render_mode=None, local_vis=True, size=100, width=10):
        ...
        grid_obs = spaces.Box(low=0, high=1,
                            shape=(self.robots_num, obs_shape, obs_shape),
                            dtype=np.float32)
        robots = spaces.Box(low=0, high=self.width,
                            shape=(self.robots_num, 2),
                            dtype=np.float32)
    ...
    def _get_obs(self):
        total_obs = np.zeros((self.robots_num, self.obs_shape, self.obs_shape))
        ...
        dict_obs = {"grid": total_obs, "agents": robots}
    ...
    def reset(self, seed=None, options=None):
        ...
        self._robots_positions = [self.width * np.random.rand(2) for _ in range(self.robots_num)]
